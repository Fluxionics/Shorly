...
export default function Auth() {
  ...
  const { mode, redirect, url } = router.query
  ...
  return (
    <>
      <Head><title>{isSignup ? 'Sign up' : 'Sign in'} — Shorly</title></Head>
      ...
      <div style={{
        ...
        <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none', marginBottom: 40 }}>
          ...
          <span style={{ fontFamily: 'Syne', fontWeight: 800, fontSize: 24, color: 'var(--text)' }}>Shorly</span>
        </Link>
        ...
        <div className="card" style={{
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: 14, marginBottom: 28 }}>
            {isSignup ? 'Start shortening links for free today.' : 'Sign in to your Shorly dashboard.'}
          </p>
          ...
          {error && (
            <div style={{ color: 'var(--red)', fontSize: 13, marginBottom: 16, padding: '10px 14px', background: 'rgba(255,77,106,0.08)', borderRadius: 8, border: '1px solid rgba(255,77,106,0.2)' }}>
              {error}
            </div>
          )}

          {success && (
            <div style={{ color: 'var(--accent)', fontSize: 13, marginBottom: 16, padding: '10px 14px', background: 'rgba(61,255,160,0.08)', borderRadius: 8, border: '1px solid rgba(61,255,160,0.2)' }}>
              {success}
            </div>
          )}

          <button className="btn-primary" onClick={handleAuth} disabled={loading} style={{ width: '100%', opacity: loading ? 0.7 : 1 }}>
